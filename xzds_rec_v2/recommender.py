...
def embed_texts(texts):
    model = get_text_embedding_model()
    embeddings = model.encode(texts or ["empty"], normalize_embeddings=True)
...
async def upsert_item_to_qdrant(item):
    await upsert_items_to_qdrant_batch([item], batch_size=1)


async def upsert_items_to_qdrant_batch(items, batch_size: int = 64):
    if not items:
        return
    ...
    for start in range(0, len(items), batch_size):
        ...
        texts = [
            build_item_text(item["title"], item["description"], item["modality"], item.get("tags", []))
            for item in batch
        ]
