...
class ItemInitRequest(BaseModel):
    items: List[ItemData]
    batch_size: int = Field(default=ITEM_INIT_BATCH_SIZE, ge=1, le=1000)
...
async def init_items(req: ItemInitRequest):
    ...
    await upsert_items_to_qdrant_batch(item_dicts, batch_size=req.batch_size)
    return {
...
async def refresh_feed(req: EventRequest):
    ...
    async with user_refresh_locks[req.user_id]:
